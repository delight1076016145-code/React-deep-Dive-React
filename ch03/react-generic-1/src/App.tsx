...
const App = () => {
  ...
  const refOwner = useRef<HTMLInputElement | null>(null);
  const { error, isLoading, response, requestData } = useFetch<TodoItemType[]>(`/todolist_long/${owner}`, { timeout: 5000 });

  const setOwnerHandler = () => {
    const newOwner = refOwner.current?.value;
  ...
  return (
    <div>
      사용자명 : <input type="text" defaultValue={owner} ref={refOwner} />
      <button onClick={setOwnerHandler}>설정</button>
      <hr />
