...
export function App() {
  ...
  return (
    <div style={{ padding: '20px', border: '2px solid blue' }}>
      ...
      <p>
        피더레이션 상태 : {isInHost ? '호스트앱에서 피더레이션! ' : '독립 실행'}
      </p>
      ...
      >
        {isInHost ? (
          <>
            <Link to="/app1/dashboard" style={{ marginRight: '10px' }}>
              대시보드
            </Link>
            <Link to="/app1/profile" style={{ marginRight: '10px' }}>
              프로필
            </Link>
            <Link to="/app1/settings" style={{ marginRight: '10px' }}>
              설정
            </Link>
          </>
        ) : (
